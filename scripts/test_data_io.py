...
from scripts._data_io import (load_latest_state, load_snapshot_data,
                              save_latest_state, save_snapshot_data)
...
class TestDataIO(unittest.TestCase):
    ...
    def test_save_and_load_snapshot_data(self):
        ...
        self.assertTrue(
            self.data_file.exists(), "The data file should be created on disk."
        )

        # Read back to guarantee fidelity.
        loaded = load_snapshot_data(self.data_file)
        self.assertEqual(
            loaded["snapshots"], snapshots, "Snapshots altered during save/load."
        )
        self.assertEqual(
            loaded["fossils"], fossils, "Fossils altered during save/load."
        )
    ...
    def test_save_and_load_latest_state(self):
        ...
        self.assertTrue(
            self.state_file.exists(), "The state file should be created on disk."
        )

        loaded_commit, loaded_comps = load_latest_state(self.state_file)
        self.assertEqual(
            loaded_commit, commit_hash, "Commit hash altered during save/load."
        )
        self.assertEqual(
            loaded_comps,
            file_compositions,
            "File compositions altered during save/load.",
        )
    ...
    def test_load_latest_state_missing(self):
        ...
        loaded_commit, loaded_comps = load_latest_state(self.state_file)
        self.assertIsNone(
            loaded_commit, "Missing state should return None for the commit."
        )
        self.assertIsNone(
            loaded_comps, "Missing state should return None for the compositions."
        )
